...
# from src.de_pet_project.exchange_connector.utils.message_processor import MessageProcessor
    ...
    async def receive(self) -> None:
        while self.connected:
            try:
                ...
                ts_received: float = time.time() + self.offset
                ...
                if 'id' in message:
                    ...
                    logger.info(
                        f'''
                        Round trip data:
                        client_send: {datetime.fromtimestamp(self.round_trip_data["client_send"]).strftime("%Y-%m-%d %H:%M:%S.%f")}
                        server_recv: {datetime.fromtimestamp(self.round_trip_data["server_recv"]).strftime("%Y-%m-%d %H:%M:%S.%f")}
                        server_send: {datetime.fromtimestamp(self.round_trip_data['server_send']).strftime("%Y-%m-%d %H:%M:%S.%f")}
                        client_recv: {datetime.fromtimestamp(self.round_trip_data['client_recv']).strftime("%Y-%m-%d %H:%M:%S.%f")}
                        '''
                    )
